...
function (Okta, Q, Logger, OktaAuth, Util, Expect, Router,
  ...
  Expect.describe('LoginRouter', function () {
    ...
    itp('triggers an afterNavigation event when routing to default route and when status is UNAUTHENTICATED', function () {
      return setup({ stateToken: 'aStateToken' })
      .then(function (test) {
        Util.mockRouterNavigate(test.router);
        test.setNextResponse(resUnauthenticated);
      ...
      .then(function (test) {
        ...
        expect(test.afterNavSpy.calls.allArgs()[0]).toEqual([{ controller: 'refresh-auth-state' }]);
        expect(test.afterNavSpy.calls.allArgs()[1]).toEqual([{ controller: 'primary-auth' }]);
      });
    });
    ...
    Expect.describe('Events', function () {
      ...
      itp('triggers an afterNavigation event when first controller is loaded', function() {
        return setup()
        .then(function (test) {
          test.router.primaryAuth();
          return Expect.waitForPrimaryAuth(test);
        ...
        .then(function(test){
          expect(test.afterNavSpy.calls.count()).toBe(1);
          expect(test.afterNavSpy).toHaveBeenCalledWith({ controller: 'primary-auth' });
        });
      });
      itp('triggers both pageRendered and afterNavigation events when first controller is loaded', function() {
        return setup()
        .then(function (test) {
          test.router.primaryAuth();
          return Expect.waitForPrimaryAuth(test);
      ...
      itp('triggers an afterNavigation event when navigating to a new controller', function() {
        return setup()
        .then(function (test) {
          // Test navigation from primary Auth to Forgot password page
          test.router.primaryAuth();
        ...
        .then(function(test) {
          expect(test.afterNavSpy.calls.count()).toBe(1);
          expect(test.afterNavSpy.calls.allArgs()[0]).toEqual([{ controller: 'primary-auth' }]);
          Util.mockRouterNavigate(test.router);
          test.router.navigate('signin/forgot-password');
          return Expect.waitForForgotPassword(test);
        })
