...
```javascript
// Handle a 'afterNavigation' event using an onNavigated callback
signIn.on('afterNavigation', onNavigated);
```
...
// Unsubscribe all listeners that have been registered to the 'afterNavigation' event
signIn.off('afterNavigation');

// Unsubscribe the onNavigated listener from the 'afterNavigation' event
signIn.off('afterNavigation', onNavigated);
```
...
signIn.on('afterNavigation', function (context) {
...
:warning: This event has been *deprecated*, please use [**afterNavigation**](#afternavigation) instead.
